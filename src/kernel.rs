pub mod kernel {
    ...
    pub extern "C" fn unpack_img(file_name: *const c_char, _n: bool, _h: bool) -> *mut c_char {
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot unpack {} {} {}",
            a,
            b,
            cstring_to_string(file_name)
        )));
        match a.success {
            true => {
                // 如果命令执行成功，则返回 "OK"
    ...
    pub extern "C" fn repack_img(
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot repack {} {} {}",
            a,
            cstring_to_string(origboot),
            cstring_to_string(out_file_name)
        )));
        match a.success {
            true => {
                // 如果命令执行成功，则返回 "OK"
    ...
    pub extern "C" fn verify(file: *const c_char, pom: *const c_char) -> *const c_char {
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot verify {} {} ",
            cstring_to_string(file),
            cstring_to_string(pom)
        )));
        match a.success {
            true => {
                // 如果命令执行成功，则返回 "OK"
    ...
    pub extern "C" fn extract(
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot extract {} {} {}",
            cstring_to_string(payload_bin),
            cstring_to_string(partition),
            cstring_to_string(outfile)
        )));
        // 根据命令执行结果返回相应的字符串
        match a.success {
            true => {
    ...
    pub extern "C" fn hexpatch(
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot hexpatch {} {} {}",
            cstring_to_string(file),
            cstring_to_string(hexpattern1),
            cstring_to_string(hexpattern2)
        )));
    ...
    pub extern "C" fn dtb(
        ...
        let a = c_exec(str_to_cstr(format!(
            "magiskboot dtb {} {} {}",
            cstring_to_string(file),
            cstring_to_string(action),
            cstring_to_string(args)
        )));
        // 根据命令执行结果返回相应的C字符串
        match a.success {
            true => {
    ...
    pub extern "C" fn hsa1(file: *const c_char) -> *const c_char {
        // 执行命令并获取结果
        let a = c_exec(str_to_cstr(format!("magiskboot hsa1 {} ", cstring_to_string(file))));
        // 根据命令执行结果返回相应的C字符串
        match a.success {
            true => {
                // 如果命令执行成功，则返回 "OK"
    ...
    pub extern "C" fn magisk_clean() -> *const c_char {
        ...
        let a = c_exec(str_to_cstr("magiskboot cleanup"));
