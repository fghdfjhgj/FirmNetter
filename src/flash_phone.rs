...
pub mod flash_phone {
    ...
    pub extern "C" fn free_no_root_phone_data(data: &mut NoRootPhoneData) {
        ...
        unsafe {
            ...
            if !data.kernel_version.is_null() {
                let _ = CString::from_raw(data.kernel_version as *mut c_char);
            }
            if !data.android_version.is_null() {
                let _ = CString::from_raw(data.android_version as *mut c_char);
            }
            if !data.sdk_version.is_null() {
                let _ = CString::from_raw(data.sdk_version as *mut c_char);
            }
            if !data.ro_product_manufacturer.is_null() {
                let _ = CString::from_raw(data.ro_product_manufacturer as *mut c_char);
            }
            if !data.ro_cpu_abi.is_null() {
                let _ = CString::from_raw(data.ro_cpu_abi as *mut c_char);
            }
            if !data.ro_product_brand.is_null() {
                let _ = CString::from_raw(data.ro_product_brand as *mut c_char);
            }
            if !data.ro_product_model.is_null() {
                let _ = CString::from_raw(data.ro_product_model as *mut c_char);
            }
            if !data.ro_product_device.is_null() {
                let _ = CString::from_raw(data.ro_product_device as *mut c_char);
            }
            if !data.ro_hardware.is_null() {
                let _ = CString::from_raw(data.ro_hardware as *mut c_char);
            }
            if !data.ro_build_description.is_null() {
                let _ = CString::from_raw(data.ro_build_description as *mut c_char);
            }
            if !data.ro_build_version_security_patch.is_null() {
                let _ = CString::from_raw(data.ro_build_version_security_patch as *mut c_char);
            }
            if !data.ro_build_id.is_null() {
                let _ = CString::from_raw(data.ro_build_id as *mut c_char);
            }
            if !data.ro_bootloader.is_null() {
                let _ = CString::from_raw(data.ro_bootloader as *mut c_char);
            }
            if !data.ro_modem_software_version.is_null() {
                let _ = CString::from_raw(data.ro_modem_software_version as *mut c_char);
            }
            if !data.ro_kernel_qemu.is_null() {
                let _ = CString::from_raw(data.ro_kernel_qemu as *mut c_char);
            }
        }
    ...
    pub extern "C" fn get_no_root_phone_data(id: *const c_char) -> *mut NoRootPhoneData {
        ...
        let properties = vec![
            (
                "kernel_version",
                format!("adb -s {} shell uname -r", id_str),
            ),
            (
                "android_version",
                format!("adb -s {} shell getprop ro.build.version.release", id_str),
            ),
            (
                "sdk_version",
                format!("adb -s {} shell getprop ro.build.version.sdk", id_str),
            ),
            (
                "ro_product_manufacturer",
                format!("adb -s {} shell getprop ro.product.manufacturer", id_str),
            ),
            (
                "ro_cpu_abi",
                format!("adb -s {} shell getprop ro.product.cpu.abi", id_str),
            ),
            (
                "ro_product_brand",
                format!("adb -s {} shell getprop ro.product.brand", id_str),
            ),
            (
                "ro_product_model",
                format!("adb -s {} shell getprop ro.product.model", id_str),
            ),
            (
                "ro_product_device",
                format!("adb -s {} shell getprop ro.product.device", id_str),
            ),
            (
                "ro_hardware",
                format!("adb -s {} shell getprop ro.hardware", id_str),
            ),
            (
                "ro_build_description",
                format!("adb -s {} shell getprop ro.build.description", id_str),
            ),
            (
                "ro_build_version_security_patch",
                format!(
                    "adb -s {} shell getprop ro.build.version.security_patch",
                    id_str
                ),
            ),
            (
                "ro_build_id",
                format!("adb -s {} shell getprop ro.build.id", id_str),
            ),
            (
                "ro_bootloader",
                format!("adb -s {} shell getprop ro.bootloader", id_str),
            ),
            (
                "ro_modem_software_version",
                format!("adb -s {} shell getprop ro.modem.software.version", id_str),
            ),
            (
                "ro_kernel_qemu",
                format!("adb -s {} shell getprop ro.kernel.qemu", id_str),
            ),
        ];
        ...
        for (field, command) in properties {
            ...
            match field {
                ...
                "ro_product_manufacturer" => {
                    no_root_phone_data.ro_product_manufacturer = cstr.into_raw()
                }
                ...
                "ro_build_version_security_patch" => {
                    no_root_phone_data.ro_build_version_security_patch = cstr.into_raw()
                }
                "ro_build_id" => no_root_phone_data.ro_build_id = cstr.into_raw(),
                "ro_bootloader" => no_root_phone_data.ro_bootloader = cstr.into_raw(),
                "ro_modem_software_version" => {
                    no_root_phone_data.ro_modem_software_version = cstr.into_raw()
                }
    ...
    pub fn execute_fastboot_command(
        id: *const c_char,
    ...
    pub extern "C" fn flash_boot_a(id: *const c_char, path: *const c_char) -> *const c_char {
        execute_fastboot_command(id, str_to_cstr("flash boot_a"), path)
    }

    #[unsafe(no_mangle)]
    pub extern "C" fn flash_boot_b(id: *const c_char, path: *const c_char) -> *const c_char {
        execute_fastboot_command(id, str_to_cstr("flash boot_b"), path)
    }

    #[unsafe(no_mangle)]
    pub extern "C" fn flash_boot(id: *const c_char, path: *const c_char) -> *const c_char {
        execute_fastboot_command(id, str_to_cstr("flash boot"), path)
    }

    #[unsafe(no_mangle)]
    pub extern "C" fn flash_recovery(id: *const c_char, path: *const c_char) -> *const c_char {
        execute_fastboot_command(id, str_to_cstr("flash recovery"), path)
    }
    ...
    pub extern "C" fn get_kernel_version(id: *const c_char) -> *const c_char {
        execute_adb_command(id, str_to_cstr("uname"), str_to_cstr("-r"))
    }
